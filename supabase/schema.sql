...
create trigger workspace_groups_updated_at before update on workspace_groups for each row execute function set_updated_at();
alter table workspace_groups enable row level security;
create policy "Authenticated can read workspace_groups" on workspace_groups for select to authenticated using (true);
create policy "Admins can manage workspace_groups" on workspace_groups for all to authenticated using (exists (select 1 from profiles where profiles.id = auth.uid() and profiles.role = 'admin'));
...
create table if not exists meetings (
  ...
  ics_uid text, created_by uuid references public.profiles(id), updated_by uuid references public.profiles(id),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
