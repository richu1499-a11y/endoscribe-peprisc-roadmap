...
const APP_ROLE_OPTIONS = ["user", "admin"] as const;
...
export default function AdminUsersPage() {
  ...
  async function handleInvite(email: string, role: string) {
    ...
    try {
      ...
      const { error: inviteError } = await sb.from("user_invites").insert({ email: email.toLowerCase(), app_role: role, invited_by: user?.id ?? null });
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <select className={selCls} value={filterRole} onChange={e => setFilterRole(e.target.value)}>
          ...
          {APP_ROLE_OPTIONS.map(r => <option key={r}>{r}</option>)}
        </select>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 pt-24">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
        ...
        <form onSubmit={e => { e.preventDefault(); if (email.trim()) onSave(email.trim(), role); }} className="space-y-3">
          ...
          <div><label className="block text-xs font-medium text-slate-600 mb-1">Role</label><select className={c} value={role} onChange={e => setRole(e.target.value)}>{APP_ROLE_OPTIONS.map(r => <option key={r}>{r}</option>)}</select></div>
