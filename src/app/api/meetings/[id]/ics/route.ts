...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const supabase = createServerClient(url, key, {
    cookies: {
      ...
      setAll(cookiesToSet) { try { for (const { name, value, options } of cookiesToSet) { cookieStore.set(name, value, options); } } catch { /* read-only */ } },
    },
  });
  ...
  if (profile && (profile as unknown as { is_active?: boolean }).is_active === false) {
    return NextResponse.json({ error: "Account deactivated" }, { status: 403 });
  }

  // Fetch meeting (RLS also applies)
  ...
  const end = meeting.end_time ? new Date(meeting.end_time) : new Date(start.getTime() + 3600000);

  const lines = [
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//EndoScribe Workspace OS//EN",
    "CALSCALE:GREGORIAN", "METHOD:REQUEST", "BEGIN:VEVENT",
    `UID:${uid}`, `DTSTAMP:${toIcsDate(now)}`, `DTSTART:${toIcsDate(start)}`, `DTEND:${toIcsDate(end)}`,
    ...
    `SEQUENCE:${seq}`, `STATUS:CONFIRMED`,
  ];
  for (const email of (meeting.attendee_emails ?? []) as string[]) {
    lines.push(`ATTENDEE;RSVP=TRUE:mailto:${email}`);
  }
  lines.push("END:VEVENT", "END:VCALENDAR");

  await supabase.from("meetings").update({ ics_uid: uid, ics_sequence: seq, ics_last_generated_at: now.toISOString() }).eq("id", id);
